...
これら単機能の VI の他に、これらの VI を用いて作った [ユーティリティVI](../../utilities/Readme.md) もあって、いろんな測定に汎用的に使えることを期待している。

- [ユーティリティVI](../../utilities/Readme.md) のリスト
...
VI を名前で指定してコントロールを読み書きする際に同じ VI のクローンがあるときにどうなるか良く分かっていない。そもそもクローンというのが性格に何なのか、現在いくつ存在するかを確かめる手段なんかも分かっていない。。。
...
TODO: 疑似コントロールとして VI 位置や状態を設定できるようにすると、画面上の VI 配置を保存・復帰するのに便利かもしれない？
...
- Variant に入れられた配列やオブジェクトの個々の要素を読み書きするための標準的な手段は与えられていない
  ...
  - これらに対応するため [Variant 関連ライブラリ](../Variant/Readme.md) を作成する必要があった
...
TODO
--
- コントロールパスをたどる部分、すべてのコントロールを列挙してるのは無駄かもしれない？
  - VI の `GetControlIndexByName` をうまく使って効率アップを図れないか検討したい
- アプリケーション名や VI 名、コントロール名をメモ化して、二度目からは検索の必要をなくすと良さそう
